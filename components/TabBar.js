...
const TabBar = () => {
  return (
    <View style={{ position:'absolute', left:0, right:0, bottom:0, height:50, flexDirection:"row", justifyContent:"space-around"}} >
      <Button color="#434a56" style={{height:50}} title="Login" onPress={() =>{
        console.log('pressd Login')
        Actions.Login()}} >Login</Button>
      <Button color="#434a56" style={{height:50}} title="Map" onPress={() => Actions.SwarmMap()}>Map</Button>
      <Button color='darkred' style={{height:50}}  title="Alert" onPress={() => Actions.sandbox()}>Alert</Button>
    </View>
  )
}
